...
const WarehouseWeeklyReportModal = ({ analytics, isDark, onClose }) => {
  const weeklyData = useMemo(() => {
    ...
    const totalStock = weekWarehouse.reduce((sum, entry) => sum + (entry.stock || 0), 0);
    const totalMovements = weekWarehouse.reduce((sum, entry) => sum + (entry.movements || 0), 0);
    const avgAccuracy = weekWarehouse.reduce((sum, entry) => sum + (entry.accuracy || 0), 0) / weekWarehouse.length || 0;
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className={`w-full max-w-2xl p-8 rounded-xl border shadow-2xl ${
          ...
          <div className="text-center">
            ...
            <button onClick={onClose} className="mt-4 px-4 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700">
              Fermer
            </button>
          </div>
        </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className={`text-sm mt-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  Analyse complète des performances logistiques
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className={`p-6 rounded-xl border ${
                ...
                <div className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  {weeklyData.totalStock} unités en stock
                </div>
              </div>
              ...
              <div className={`p-6 rounded-xl border ${
                ...
                <div className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {[weeklyData.avgWarehouse >= 90, weeklyData.avgAccuracy >= 95].filter(Boolean).length}/2
                </div>
                <div className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  sur 2 KPIs
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <ul className={`text-sm space-y-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                    {weeklyData.avgWarehouse >= 90 && <li>• Excellente performance entrepôt</li>}
                    {weeklyData.avgAccuracy >= 95 && <li>• Inventaire très précis</li>}
                    {weeklyData.totalMovements > 0 && <li>• Activité logistique soutenue</li>}
                    {weeklyData.totalStock > 0 && <li>• Stock bien géré</li>}
                  </ul>
                </div>
                <div>
                  <h4 className={`font-medium mb-2 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                  ...
                  <ul className={`text-sm space-y-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                    ...
                    {weeklyData.avgAccuracy < 90 && <li>• Précision inventaire à revoir</li>}
                    {weeklyData.totalMovements === 0 && <li>• Aucun mouvement enregistré</li>}
                    {weeklyData.totalStock === 0 && <li>• Stock vide</li>}
                  </ul>
                </div>
                <div>
                  <h4 className={`font-medium mb-2 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                  ...
                  <ul className={`text-sm space-y-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                    <li>• Optimiser flux logistiques</li>
                    <li>• Vérifier précision inventaire</li>
                    <li>• Maintenir niveau stock</li>
                    <li>• Améliorer traçabilité</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            {departmentKPIs.length > 0 ? (
                  ...
                  {departmentKPIs.map((kpi, index) => (
                    <button
                      ...
                      className={`w-full p-6 rounded-xl border text-left transition-all duration-300 hover:scale-[1.02] ${
                        selectedKPI === kpi.id
                          ? isDark
                            ? 'border-violet-500 bg-violet-950/30 shadow-lg'
              ...
              <div className="text-center py-16">
                <div className={`w-20 h-20 rounded-2xl ${isDark ? 'bg-slate-800' : 'bg-violet-50'} flex items-center justify-center mx-auto mb-6`}>
                  <Package className={`w-10 h-10 ${isDark ? 'text-slate-600' : 'text-violet-400'}`} />
                </div>
            ...
            {selectedKPIData ? (
              <div className="space-y-8">
                <div>
                  ...
                  <div className={`p-6 rounded-xl border ${
                    isDark ? 'border-slate-600 bg-slate-800/50' : 'border-slate-200 bg-gradient-to-r from-violet-50 to-purple-50'
                  }`}>
                ...
                <div className={`p-6 rounded-xl border ${
                  isDark ? 'border-slate-600 bg-slate-800/30' : 'border-violet-200 bg-gradient-to-r from-violet-50 to-purple-50'
                }`}>
                  <div className="flex items-center space-x-4">
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center">
                  <div className={`w-24 h-24 rounded-2xl ${isDark ? 'bg-slate-800' : 'bg-violet-50'} flex items-center justify-center mx-auto mb-6`}>
                    <Target className={`w-12 h-12 ${isDark ? 'text-slate-600' : 'text-violet-400'}`} />
                  </div>
                  <h4 className={`text-xl font-bold mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="flex justify-end gap-4">
          ...
          {selectedKPI && (
            <button
              ...
              className="px-8 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white rounded-xl text-sm font-medium transition-all duration-300 shadow-lg hover:shadow-xl"
            >
              Continuer
            </button>
          )}
...
export const WarehousesPage = () => {
  ...
  const getStats = () => {
    if (!departmentSummary?.kpis) {
      return [
        { title: 'Indicateurs Suivis', value: '0/0', change: 0, changeText: 'actifs', icon: Target, color: 'violet' },
        { title: 'Performance Globale', value: '0%', change: 0, changeText: 'efficacité', icon: TrendingUp, color: 'violet' },
    ...
    <div className={`space-y-8 ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Contrôle des stocks, traçabilité et gestion logistique avancée
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button 
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl hover:shadow-lg hover:from-violet-700 hover:to-purple-700 transition-all duration-200"
          >
        ...
        {stats.map((stat, index) => (
          ...
          >
            <div className={`absolute inset-0 bg-gradient-to-br ${
              stat.color === 'violet' ? (isDark ? 'from-violet-500/10 to-purple-500/5' : 'from-violet-50 to-purple-50/50') :
      ...
      <div className={`rounded-2xl border p-6 ${
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => handleAddData()}
            className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl hover:from-violet-700 hover:to-purple-700 transition-all duration-300 text-sm font-medium shadow-lg"
          >
            <Plus className="w-4 h-4" />
        ...
        {departmentSummary?.kpis && departmentSummary.kpis.length > 0 ? (
            ...
            {departmentSummary.kpis.map((kpi) => {
              const getStatusColor = (status) => {
                switch (status) {
                  ...
                  case 'good': return 'text-violet-600 bg-violet-50 dark:bg-violet-900/20';
              ...
              return (
                <div
                  ...
                  className={`group cursor-pointer p-6 rounded-2xl border transition-all duration-300 hover:shadow-xl hover:scale-[1.02] ${
                    isDark ? 'bg-slate-800/60 border-slate-700/50 hover:border-violet-500/50' : 'bg-white border-slate-200/80 hover:border-violet-500/50 shadow-sm'
                  }`}
                >
                  ...
                  {kpi.latestValue ? (
                    ...
                    <div className="text-center py-6">
                      <div className={`w-12 h-12 rounded-xl ${
                        isDark ? 'bg-slate-700 group-hover:bg-violet-900/30' : 'bg-slate-100 group-hover:bg-violet-50'
                      } flex items-center justify-center mx-auto mb-3 transition-all`}>
                        <Plus className={`w-6 h-6 ${
                          isDark ? 'text-slate-500 group-hover:text-violet-400' : 'text-slate-400 group-hover:text-violet-500'
                        } transition-colors`} />
                      </div>
                      <p className={`text-sm font-medium ${
                        isDark ? 'text-slate-300 group-hover:text-violet-300' : 'text-slate-600 group-hover:text-violet-600'
                      } transition-colors`}>
          ...
          <div className="text-center py-12">
            <div className={`w-16 h-16 rounded-2xl ${isDark ? 'bg-slate-700/50' : 'bg-violet-50'} flex items-center justify-center mx-auto mb-4`}>
              <Package className="w-8 h-8 text-violet-500" />
            </div>
            <h4 className={`text-lg font-semibold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              Prêt à Démarrer la Logistique
            </h4>
            <p className={`text-sm mb-6 max-w-md mx-auto ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Commencez à ajouter des données d'indicateurs pour votre entrepôt afin de suivre les stocks et la logistique
            </p>
            <button
              onClick={() => handleAddData()}
              className="px-6 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl hover:from-violet-700 hover:to-purple-700 transition-all font-medium shadow-lg hover:shadow-xl"
            >
