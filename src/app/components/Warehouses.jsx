        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              ...
              <Package className={`w-5 h-5 ${isDark ? 'text-slate-400' : 'text-slate-600'}`} />
            </div>
            <div>
              <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {t.selectWarehouseKpi || 'Select Warehouse KPI'}
              </h3>
              <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                {t.selectWarehouseKpiDescription || 'Choose the warehouse KPI to update'}
              </p>
            </div>
          </div>
            ...
            {departmentKPIs.length > 0 ? (
                  ...
                  {departmentKPIs.map((kpi, index) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        {selectedKPI === kpi.id && (
                          <CheckCircle2 className="w-5 h-5 text-violet-500 mt-1" />
                        )}
                      </div>
                    </button>
                  ))}
              ...
              <div className="text-center py-12">
                <div className={`w-16 h-16 rounded-lg ${isDark ? 'bg-slate-800' : 'bg-slate-100'} flex items-center justify-center mx-auto mb-4`}>
                  <Package className={`w-8 h-8 ${isDark ? 'text-slate-600' : 'text-slate-400'}`} />
                </div>
                <p className={`text-base font-medium mb-2 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                  {t.noWarehouseKpisConfigured || 'No Warehouse KPIs configured'}
                </p>
...
export const WarehousesPage = () => {
  ...
  // Get warehouse statistics with real data
  const getStats = () => {
    if (!departmentSummary?.kpis) {
      return [
        { title: t.kpisTracked || 'KPIs Tracked', value: '0/0', change: 0, changeText: 'active', icon: Target, color: 'violet' },
        { title: t.performance || 'Performance', value: '0%', change: 0, changeText: 'efficiency', icon: TrendingUp, color: 'violet' },
        { title: t.excellent || 'Excellent', value: '0', change: 0, changeText: 'KPIs', icon: CheckCircle, color: 'violet' },
  ...
  const deptName = department?.name?.[language] || department?.name?.en || 'Warehouses & Logistics';

  // Debug localStorage on component mount
  React.useEffect(() => {
    console.log('Warehouse Page mounted. Current localStorage data:', localStorage.getItem('hydrachim_kpi_data'));
    ...
    <div className={`space-y-8 ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              {t.warehousesDescription || 'Inventory control, traceability, and logistics management'}
            </p>
          </div>
        </div>
        <button 
          onClick={() => handleAddData()}
          className="flex items-center space-x-2 px-6 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl hover:from-violet-700 hover:to-purple-700 transition-all duration-300 font-medium shadow-lg hover:shadow-xl"
        >
          <Plus className="w-4 h-4" />
          <span>{t.addInventoryData || 'Add Inventory Data'}</span>
        </button>
      </div>
              ...
              <div className="relative">
                ...
                <div>
                  ...
                  <p className={`text-3xl font-bold mb-3 ${
                    stat.color === 'violet' ? 'text-violet-600' : 'text-slate-600'
                  }`}>
                    {stat.value}
                  </p>
      ...
      <div className={`rounded-2xl border p-6 ${
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button 
            onClick={() => handleAddData()}
            className="flex items-center space-x-2 px-4 py-2 bg-violet-600 text-white rounded-xl hover:bg-violet-700 transition-colors text-sm font-medium"
          >
            <Plus className="w-4 h-4" />
        ...
        {departmentSummary?.kpis && departmentSummary.kpis.length > 0 ? (
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className={`px-2 py-1 rounded text-xs font-medium transition-all ${
                        isDark 
                          ? 'bg-slate-700 text-slate-300 group-hover:bg-violet-900/30 group-hover:text-violet-300' 
          ...
          <div className="text-center py-12">
            ...
            <h4 className={`text-lg font-semibold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {t.noDataAvailable_warehouse || 'No Data Available'}
            </h4>
            <p className={`text-sm mb-6 max-w-md mx-auto ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              {t.startAddingKpiData || 'Start adding KPI data for your warehouse to track inventory and logistics'}
            </p>
            <button 
              onClick={() => handleAddData()}
