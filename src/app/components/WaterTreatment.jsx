...
// Enhanced Water Treatment KPI Selector Component
const WaterKPISelector = ({ onSelect, onCancel, isDark, departmentKPIs }) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              ...
              <Droplets className={`w-5 h-5 ${isDark ? 'text-slate-400' : 'text-slate-600'}`} />
            </div>
            <div>
              <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {t.selectWaterKpi || 'Select Water Treatment KPI'}
              </h3>
              <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                {t.selectWaterKpiDescription || 'Choose the water treatment KPI to update'}
              </p>
            </div>
          </div>
            ...
            {departmentKPIs.length > 0 ? (
                  ...
                  {departmentKPIs.map((kpi, index) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        {selectedKPI === kpi.id && (
                          <CheckCircle2 className="w-5 h-5 text-cyan-500 mt-1" />
                        )}
                      </div>
                    </button>
                  ))}
              ...
              <div className="text-center py-12">
                <div className={`w-16 h-16 rounded-lg ${isDark ? 'bg-slate-800' : 'bg-slate-100'} flex items-center justify-center mx-auto mb-4`}>
                  <Droplets className={`w-8 h-8 ${isDark ? 'text-slate-600' : 'text-slate-400'}`} />
                </div>
                <p className={`text-base font-medium mb-2 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                  {t.noWaterKpisConfigured || 'No Water Treatment KPIs configured'}
                </p>
...
export const WaterTreatmentPage = () => {
  ...
  const departmentId = 'waterTreatment';
  ...
  // Get water treatment statistics with real data
  const getStats = () => {
    if (!departmentSummary?.kpis) {
      return [
        { title: t.kpisTracked || 'KPIs Tracked', value: '0/0', change: 0, changeText: 'active', icon: Target, color: 'cyan' },
        { title: t.performance || 'Performance', value: '0%', change: 0, changeText: 'efficiency', icon: TrendingUp, color: 'cyan' },
        { title: t.excellent || 'Excellent', value: '0', change: 0, changeText: 'KPIs', icon: CheckCircle, color: 'cyan' },
  ...
  const deptName = department?.name?.[language] || department?.name?.en || 'Water Treatment & Washing';

  // Debug localStorage on component mount
  React.useEffect(() => {
    console.log('Water Treatment Page mounted. Current localStorage data:', localStorage.getItem('hydrachim_kpi_data'));
    ...
    <div className={`space-y-8 ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              {t.waterTreatmentDescription || 'Utility efficiency and hygiene standards monitoring'}
            </p>
          </div>
        </div>
        <button 
          onClick={() => handleAddData()}
          className="flex items-center space-x-2 px-6 py-2.5 bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all duration-300 font-medium shadow-lg hover:shadow-xl"
        >
          <Plus className="w-4 h-4" />
          <span>{t.addMeasurement || 'Add Measurement'}</span>
        </button>
      </div>
          ...
          return (
            ...
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${
                stat.color === 'cyan' ? (isDark ? 'from-cyan-500/10 to-blue-500/5' : 'from-cyan-50 to-blue-50/50') :
              ...
              <div className="relative">
                ...
                <div>
                  ...
                  <p className={`text-3xl font-bold mb-3 ${
                    stat.color === 'cyan' ? 'text-cyan-600' : 'text-slate-600'
                  }`}>
                    {stat.value}
                  </p>
      ...
      <div className={`rounded-2xl border p-6 ${
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button 
            onClick={() => handleAddData()}
            className="flex items-center space-x-2 px-4 py-2 bg-cyan-600 text-white rounded-xl hover:bg-cyan-700 transition-colors text-sm font-medium"
          >
            <Plus className="w-4 h-4" />
        ...
        {departmentSummary?.kpis && departmentSummary.kpis.length > 0 ? (
            ...
            {departmentSummary.kpis.map((kpi) => {
              const getStatusColor = (status) => {
                switch (status) {
                  ...
                  case 'good': return 'text-cyan-600 bg-cyan-50 dark:bg-cyan-900/20';
              ...
              return (
                <div
                  ...
                  className={`group cursor-pointer p-6 rounded-2xl border transition-all duration-300 hover:shadow-xl hover:scale-[1.02] ${
                    isDark ? 'bg-slate-800/60 border-slate-700/50 hover:border-cyan-500/50' : 'bg-white border-slate-200/80 hover:border-cyan-500/50 shadow-sm'
                  }`}
                >
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className={`px-2 py-1 rounded text-xs font-medium transition-all ${
                        isDark 
                          ? 'bg-slate-700 text-slate-300 group-hover:bg-cyan-900/30 group-hover:text-cyan-300' 
                  ...
                  {kpi.latestValue ? (
                    ...
                    <div className="text-center py-6">
                      <div className={`w-12 h-12 rounded-xl ${
                        isDark ? 'bg-slate-700 group-hover:bg-cyan-900/30' : 'bg-slate-100 group-hover:bg-cyan-50'
                      } flex items-center justify-center mx-auto mb-3 transition-all`}>
                        <Plus className={`w-6 h-6 ${
                          isDark ? 'text-slate-500 group-hover:text-cyan-400' : 'text-slate-400 group-hover:text-cyan-500'
                        } transition-colors`} />
                      </div>
                      <p className={`text-sm font-medium ${
                        isDark ? 'text-slate-300 group-hover:text-cyan-300' : 'text-slate-600 group-hover:text-cyan-600'
                      } transition-colors`}>
          ...
          <div className="text-center py-12">
            <div className={`w-16 h-16 rounded-2xl ${isDark ? 'bg-slate-700/50' : 'bg-cyan-50'} flex items-center justify-center mx-auto mb-4`}>
              <Droplets className="w-8 h-8 text-cyan-500" />
            </div>
            <h4 className={`text-lg font-semibold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {t.noDataAvailable_water || 'No Data Available'}
            </h4>
            <p className={`text-sm mb-6 max-w-md mx-auto ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              {t.startAddingKpiData || 'Start adding KPI data for your water treatment to track quality and efficiency'}
            </p>
            <button 
              onClick={() => handleAddData()}
              className="px-6 py-2.5 bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all font-medium shadow-lg hover:shadow-xl"
            >
